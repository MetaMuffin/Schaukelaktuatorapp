...
class AppSchwingungsverlaufState extends State<AppSchwingungsverlauf> {
  LineChartData plotData = LineChartData();
  ...
  void initState() {
    ...
    wse.on("rt_data", context, (ev, data) {
      ...
      List<double> ddata =
          sdata.split(",").map((v) => double.parse(v)).toList();
      updatePlot(new RTDataVelPwm(
          DateTime.fromMillisecondsSinceEpoch(ddata[0].toInt()),
          ddata[1],
          ddata[2],
          ddata[3]));
    });
  ...
  void updatePlot(RTDataVelPwm newAmpltudeData) {
    ...
    // amplitudeData =
    //     amplitudeData.sublist(max(amplitudeData.length - maxDataLen, 0));
